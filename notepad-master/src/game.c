...
void game_tick(PacmanGame *game) {
	...
	case GamePlayState:
		...
		if (game->pacman[0].score > game->highscore)
			game->highscore = game->pacman[0].score;
...
static void process_pellets(PacmanGame *game) {
	...
	for (int i = 0; i < holder->totalNum; i++) {
		...
		if (game->playMode==1&&collides_obj(&game->pacman[1].body, p->x, p->y)) {
			holder->numLeft--;

			p->eaten = true;
			game->pacman[1].score += pellet_points(p);
			if (pellet_check(p)) {
				game->pacman[1].godMode = true;
